...
// 单独导出原始开关，后台只编辑纯数据；运行时解析放在下方，避免可视化编辑器处理函数表达式。
export const displaySettingsOptions: DisplaySettingsConfig = {
	// ── 总开关 (Master switch) ────────────────────────────
...
export const displaySettingsConfig: DisplaySettingsConfig =
	resolveDisplaySettingsConfig(displaySettingsOptions);
