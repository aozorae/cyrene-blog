...
管理界面使用原生 HTML、CSS 和 ES Modules，并按真实页面拆分：

- `public/*.html`：登录、仪表盘、文章、动态、配置、草稿和记录等独立页面。
- `public/pages/`：每个页面自己的初始化、事件和数据加载逻辑。
- `public/core/`：API、草稿、页面会话与通用界面工具。
- `public/layout/`：所有后台页面共用的侧栏和顶部栏。
- `public/features/`：可跨页面复用的内容列表与记录渲染。
- `public/styles/`：按基础、布局、表单、配置工作室和响应式规则拆分的样式。

页面使用显式 `.html` 地址，确保 Cloudflare 静态资源在直接访问或刷新时不依赖额外路由回退。
