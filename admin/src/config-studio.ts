...
export const STUDIO_CONFIGS = [
	...
	},
	{
		...
		title: "评论系统",
		description: "Twikoo、Waline、Giscus、Disqus 和 Artalk。",
	},
	{
		path: "src/config/analyticsConfig.ts",
		title: "统计分析",
		description: "Google Analytics、Umami、Clarity 和 51.la。",
	},
	{
		path: "src/config/fontConfig.ts",
		title: "字体",
		description: "全局字体、代码字体和字体子集化。",
	},
	{
		path: "src/config/coverImageConfig.ts",
		title: "文章封面",
		description: "文章封面图和随机图片 API。",
	},
	{
	...
	},
	{
		...
		title: "友链",
		description: "友链页面和友链列表。",
	},
	{
		path: "src/config/galleryConfig.ts",
		title: "相册",
		description: "相册页面的展示和数据源。",
	},
	{
		path: "src/config/sponsorConfig.ts",
		title: "打赏",
		description: "打赏方式与打赏页面行为。",
	},
	{
...
class ValueParser {
	...
	private parseObject(): Record<string, unknown> {
		...
		if (this.source[this.index] !== "}")
			throw new HttpError(500, "CONFIG_PARSE_ERROR", "配置对象没有正确闭合。");
		this.index += 1;
		this.skipAssertion();
		return result;
	}
	...
	private parseArray(): unknown[] {
		...
		if (this.source[this.index] !== "]")
			throw new HttpError(500, "CONFIG_PARSE_ERROR", "配置数组没有正确闭合。");
		this.index += 1;
		this.skipAssertion();
		return result;
	}
...
function parseExports(source: string): ParsedExport[] {
	...
	while ((match = pattern.exec(source))) {
		...
		result.push({
			name: match[1],
			start,
			end: expressionStart + parsed.end,
			value: parsed.value,
		});
...
function serialize(value: unknown, depth = 0): string {
	if (depth > 12)
		throw new HttpError(400, "CONFIG_TOO_DEEP", "配置层级过深，无法保存。");
...
function validateStudioValue(value: unknown, depth = 0): void {
	...
	if (typeof value === "string" && value.length > 100_000)
		throw new HttpError(400, "CONFIG_TOO_LARGE", "配置字符串过长，无法保存。");
	if (Array.isArray(value)) {
		if (value.length > 500)
			throw new HttpError(
				400,
	...
	if (value && typeof value === "object") {
		...
		if (entries.length > 300)
			throw new HttpError(400, "CONFIG_TOO_LARGE", "配置字段过多，无法保存。");
...
export async function readStudio(
	...
	const files = await Promise.all(
		STUDIO_CONFIGS.map((config) => readFile(context, config.path)),
	);
		...
		try {
			...
			if (error instanceof HttpError)
				throw new HttpError(
					error.status,
					error.code,
					`${STUDIO_CONFIGS[index].path}：${error.message}`,
				);
			throw error;
		}
	});
...
export async function buildStudioChange(
	context: RepositoryContext,
	path: string,
	values: Record<string, unknown>,
): Promise<{ change: GitChange; sha: string }> {
	validateStudioValue(values);
	if (JSON.stringify(values).length > 250_000)
		throw new HttpError(413, "CONFIG_TOO_LARGE", "配置内容过大，无法保存。");
	...
	if ("text" in config && config.text) {
		if (typeof values.content !== "string" || values.content.length > 100_000)
			throw new HttpError(
				400,
				"CONFIG_INVALID",
				"页脚内容为空或超出大小限制。",
	...
	for (const item of replacements) {
		...
		const leading =
			file.content.slice(expressionStart, item.end).match(/^\s*/)?.[0] || "";
