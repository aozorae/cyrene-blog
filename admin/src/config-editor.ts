...
function readString(source: string, key: string, fallback = ""): string {
	const match = source.match(
		new RegExp(
			`^[\\t ]*${configKeyPattern(key)}[\\t ]*:[\\t ]*(${STRING_LITERAL_PATTERN})`,
			"m",
		),
	);
...
function replaceString(source: string, key: string, value: string): string {
	...
	return source.replace(
		pattern,
		(_match, prefix: string) => `${prefix}${jsonString(value)}`,
	);
}
...
function findDelimitedBlock(
	...
	const declaration = `(?:export[\\t ]+)?const[\\t ]+${escapeRegExp(key)}(?:[\\t ]*:[^=;]+)?[\\t ]*=`;
	const match = new RegExp(
		`^[\\t ]*(?:${configKeyPattern(key)}[\\t ]*:|${declaration})[\\t ]*\\${open}`,
		"m",
	).exec(source);
...
function replaceBooleanInPages(
	source: string,
	key: string,
	value: boolean,
): string {
	const pages =
		findDelimitedBlock(source, "pageToggles", "{", "}") ||
		findDelimitedBlock(source, "pages", "{", "}");
	if (!pages)
		throw new HttpError(
			500,
			"CONFIG_SHAPE_CHANGED",
			"站点页面开关配置结构发生变化，无法安全更新。",
		);
	const before = source.slice(0, pages.start);
	const block = source.slice(pages.start, pages.end);
	const after = source.slice(pages.end);
	const pattern = new RegExp(
		`(^[\\t ]*${configKeyPattern(key)}[\\t ]*:[\\t ]*)(true|false)`,
		"m",
	);
	...
	return (
		before +
		block.replace(pattern, (_match, prefix: string) => `${prefix}${value}`) +
		after
	);
}
...
function parseSponsorMethods(source: string): SponsorMethodInput[] {
	...
	try {
		const parsed = JSON.parse(arraySource) as Array<
			Partial<SponsorMethodInput>
		>;
		...
		return (arraySource.match(/\{[\s\S]*?\}/g) || [])
			.map((item) => ({
...
export function parseSettings(
	...
	const pagesBlock =
		findDelimitedBlock(site, "pageToggles", "{", "}") ||
		findDelimitedBlock(site, "pages", "{", "}");
	const pagesSource = pagesBlock
		? site.slice(pagesBlock.start, pagesBlock.end)
		: "";
	for (const page of [
		"friends",
		"sponsor",
		...
		const match = pagesSource.match(
			new RegExp(
				`^[\\t ]*${configKeyPattern(page)}[\\t ]*:[\\t ]*(true|false)`,
				"m",
			),
		);
		pages[page] = match?.[1] === "true";
	}
	const githubBlock = profile.match(
		new RegExp(
			`\\{[^}]*${configKeyPattern("name")}[\\t ]*:[\\t ]*["']GitHub["'][^}]*${configKeyPattern("url")}[\\t ]*:[\\t ]*(${STRING_LITERAL_PATTERN})`,
			"m",
		),
	);
	return {
...
export function updateSiteConfig(source: string, input: SiteSettings): string {
	...
	for (const [key, value] of Object.entries(input.pages))
		updated = replaceBooleanInPages(updated, key, Boolean(value));
	return updated;
}
...
export function updateProfileConfig(
	...
	if (github.test(updated))
		updated = updated.replace(
			github,
			(_match, prefix: string) => `${prefix}${jsonString(input.githubUrl)}`,
		);
	return updated;
}
	...
	return [
		{
			...
			content: updateSiteConfig(current.site.content, input),
			sha: current.site.sha,
		},
		{
			path: current.profile.path,
			content: updateProfileConfig(current.profile.content, input),
			sha: current.profile.sha,
		},
		{
			path: current.announcement.path,
			content: updateAnnouncementConfig(current.announcement.content, input),
			sha: current.announcement.sha,
		},
		{
			path: current.sponsor.path,
			content: updateSponsorConfig(
				current.sponsor.content,
				input.sponsorMethods,
			),
			sha: current.sponsor.sha,
		},
	];
}
...
export function updateSponsorConfig(
	source: string,
	methods: SponsorMethodInput[],
): string {
	...
	const formatted = lines
		.map((line, index) => (index === 0 ? line : `${block.indent}${line}`))
		.join("\n");
		...
		directory
			...
			.some(
				(part) =>
					...
					part === "." ||
					part === ".." ||
					!/^[a-z0-9\u4e00-\u9fff_-]+$/i.test(part),
			)
	) {
		throw new HttpError(
			400,
			"ARTICLE_DIRECTORY_INVALID",
			"文章目录只能包含字母、数字、中文、短横线、下划线和斜杠。",
		);
	}
	return directory;
}
...
function mergeArticleFrontmatter(
	source: string | undefined,
	values: Record<(typeof ARTICLE_FRONTMATTER_KEYS)[number], string | null>,
): string[] {
	...
	for (const line of lines) {
		...
		const key = match?.[1] as
			| (typeof ARTICLE_FRONTMATTER_KEYS)[number]
			| undefined;
		if (!key || !ARTICLE_FRONTMATTER_KEYS.includes(key)) {
			output.push(line);
			continue;
		}
		if (replaced.has(key)) continue;
		replaced.add(key);
		if (values[key] !== null) output.push(`${key}: ${values[key]}`);
	}
	for (const key of ARTICLE_FRONTMATTER_KEYS) {
		if (!replaced.has(key) && values[key] !== null)
...
export function buildArticle(input: ArticleInput): {
	...
	if (!title || !content)
		throw new HttpError(
			400,
			"ARTICLE_REQUIRED",
			"文章标题和正文不能为空。原因：空文章无法生成有效页面。`content` 应包含 Markdown 正文。",
		);
	if (title.length > 120 || content.length > 120_000)
		throw new HttpError(
			400,
			"ARTICLE_TOO_LARGE",
			"文章标题或正文超出限制，请缩短内容后重试。",
		);
	...
	const image =
		typeof input.image === "string" ? input.image.trim().slice(0, 500) : "";
	const frontmatter = [
		"---",
		...mergeArticleFrontmatter(input.frontmatter, {
			title: jsonString(title),
			description: jsonString(description),
			published: normalizePublished(input.published),
			tags: JSON.stringify(tags),
			category: jsonString(category || "未分类"),
			image: image ? jsonString(image) : null,
	...
	return {
		path: `src/content/posts/${directory ? `${directory}/` : ""}${slug}.md`,
		content: frontmatter,
		slug,
	};
}
