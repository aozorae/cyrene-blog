...
async function findConflicts(
	...
	const checks = drafts.flatMap((draft) =>
		Object.entries(draft.baseRevision).map(([path, expectedSha]) => ({
			draftId: draft.id,
			path,
			expectedSha,
		})),
...
async function materializeDraft(
	...
	if (draft.kind === "settings")
		return buildSettingsChanges(context, payload.settings);
	if (draft.kind === "studio") {
		if (
			...
			throw new HttpError(400, "DRAFT_INVALID", "配置草稿格式无效。");
		return [
			(
				await buildStudioChange(
					context,
					payload.path,
...
export async function submitDrafts(
	...
	ids: string[],
	force: boolean,
): Promise<{ commit: { sha: string; url: string }; message: string }> {
	const uniqueIds = [...new Set(ids)].filter(
		(id) => typeof id === "string" && id.length <= 300,
	);
	if (uniqueIds.length === 0 || uniqueIds.length > 30)
		throw new HttpError(
			400,
			"DRAFT_SELECTION_INVALID",
			"请选择 1 到 30 个待提交草稿。",
		);
	...
	if (conflicts.length > 0 && !force) {
		throw new HttpError(
			409,
			"DRAFT_CONFLICT",
			"GitHub 中的文件已发生变化。你可以刷新内容，或确认后强制覆盖。",
			{ conflicts },
		);
	}
	const groups = await Promise.all(
		drafts.map((draft) => materializeDraft(context, draft)),
	);
	...
	const commit = await commitChanges(
		...
		changes,
		drafts.length === 1
			? `Update ${drafts[0].title} from Cyrene Admin`
			: `Publish ${drafts.length} staged changes from Cyrene Admin`,
		head,
	);
	await deleteDrafts(env, uniqueIds);
	await writeAudit(
		env,
		force ? "draft.force_publish" : "draft.publish",
		`${force ? "强制覆盖并" : ""}提交 ${drafts.length} 个暂存更改`,
		commit.sha,
	);
